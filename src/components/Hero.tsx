  ...
  return (
    <section className="relative min-h-screen overflow-hidden">
      <div className="absolute inset-0">
        <img src="/images/hero-bg.jpg" alt="Hero background" className="h-full w-full object-cover blur-sm" />
      ...
      <div className="container relative mx-auto flex min-h-screen flex-col items-center justify-center px-4 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="mb-6 max-w-4xl font-serif text-4xl text-white md:text-6xl lg:text-7xl leading-tight">
          {t.hero.title1}<br /><span className="text-accent">{t.hero.title2}</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          className="mb-10 max-w-2xl text-lg text-white/80">{t.hero.description}</motion.p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="flex flex-col sm:flex-row gap-4">
          ...
          <Button asChild variant="outline" size="lg" className="gap-2 border-white/30 bg-white/10 text-white hover:bg-white/20 hover:text-white">
            <a href="mailto:mfrenovation25@gmail.com">
        ...
        <motion.a href="#a-propos" animate={{ y: [0, 8, 0] }} transition={{ duration: 1.5, repeat: Infinity }}
